...
odoo.define("pos_event_sale.EventSelectorPopup", function(require) {
    ...
    const EventSelectorPopup = PopupWidget.extend({
        ...
        show: function(options) {
            ...
            this.pos.db
                .update_event_seats_available(event_ids, {timeout: 1000, shadow: true})
                .catch(errors => {
                    console.warning(
                        "Unable to update available seats from server. Using cached availability."
                    );
        ...
        renderEventSelector: function(events) {
            ...
            for (const event of events) {
                // Compute event available seats
                let seats_available = 0;
                if (event.event_ticket_ids && event.event_ticket_ids.length) {
        ...
        click_event: function(ev) {
            ...
            // A final availability check is done before paying the order.
            this.pos.db
                .update_event_seats_available([event_id], {timeout: 1000, shadow: true})
                .catch(errors => {
                    console.warning(
                        "Unable to update available seats from server. Using cached availability."
                    );
